...
function Register(){
  ...
  function switchToLogin(){
    redirect("/login", {state:location?.state})
  }
      ...
      .then(()=>{
        ...
        .then(()=>{
          ...
          const userData = {
            ...
            const result = addUserMutation.mutate(userData);
            console.log(result);
          reset({
            name:"",
            photoURL:"",
            ...
            password:"",
            captcha:""})
            sweetAlert("success", "Successfully Registered")}).catch(error=>alert(error.message))
        })
        .catch(()=>sweetAlert("error", "Error in Registrations"))
      return;
    }else{
      setError('captcha',{
        type:"manual",
  ...
  return (
    <>
      ...
      >
        ...
        <button onClick={switchToLogin} className="text-[#dca308] font-semibold">Already Registered? <span className="text-lg font-semibold tracking-wide font-cin">Go to Login</span></button>
        <p className="font-cin font-semibold text-center">or SignUp With</p>
